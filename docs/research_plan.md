...
Monte Carlo radiative transport is a class of algorithm that simulates many photons as they scatter through a medium. MCRT exists in many forms such as medical imaging (MCML, MCX, FullMonte cite) to proposed spaceborne oceanic lidar (cite). Fundamentally, these algorithms require unbiased accumulation and computing, for example even small rounding bias at full precision floating point causes decay in accuracy (github issue #41 cite), necessitating the use of many floating point intermediary values and double buffered or double precision output structures as used by MCX and FullMonte respectively (cite). This causes issues in low power environments where Field Programmable Gate Arrays (FPGA) are the only power efficient option, as many memory and compute intensive techniques have to be used. This can be solved with stochastic rounding, a technique that rounds numbers randomly, weighted by their tend towards the upper and lower bound. Stochastic rounding is highly efficient on FPGAs due to their fast and customizable RNG, and can be implemented in both the output and compute layers. The FPGA in turn allows for both layers to be optimized to arbitrarily low bit widths, which when combined allows FPGAs to increase their total performance.

Monte Carlo radiative transport is a class of algorithm which simulates many photons as they scatter through a medium and exists in many forms through fields such as medical imaging (MCML, MCX, FullMonte cite) to proposed spaceborne oceanic LIDAR (cite). Fundamentally, these algorithms require unbiased randomness, as even small rounding bias at full precision floating point causes decay in accuracy (github issue #41 cite), necessitating the use of 32 bit floating point intermediary values and double buffered or double precision output structures as used by MCX and FullMonte respectively (cite). This lends itself to the use of the Graphics Processing Unit (GPU) and its heavy hardware support of 32 bit floating point, however due its heavy use of electricity, this makes low power applications such as wearables or the aforementioned spaceborne oceanic LIDAR, systems that would normally make use of the Field Programmable Gate Array (FPGA), unviable. 
