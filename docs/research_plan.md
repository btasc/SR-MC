...
In Monte Carlo simulations, specifically voxelized Monte Carlo photon transport, referred to as VMC from now on, programs simulate many photons as they travel through some geometry, scattering at random intervals. VMC simulations are used in many fields, such as medical tissue imaging via MCX (citation) and proposed oceanic lidar deployed through satellites (citation), however all face the issue of bias. Bias occurs when the many simulated photons contribute to a total whole, specifically when that photon's contribution is so small that it causes rounding error. This problem is solved in a few ways, depending on the 

This can be solved through stochastic rounding (citation), a method of rounding that removes bias through randomly rounding up or down based on the distance between a number's lower bound and upper bound, however this type of rounding presents performance concerns on GPUs (citation), the primary hardware of choice for VMC. This necessitates the use of a Field Programmable Gate Array (FPGA), a programmable piece of hardware that allows for extreme customization in both computational performance and memory control
